...
function editNIHSSForm($patientID, $pnID) {
	...
	print "<input type='submit' value='卒中量表 (id: $pnID)' class='$buttonClass' />";
	print "</form>";
				...
				if ($query_handle2 != ""){
					...
					for ($i2 = 0; $i2 < $rows2; $i2++){
						...
						$wert2		= $data2[5];
							if (($type == 'b') And ($i2 == 0)){
								print "<div class='clear'></div>";
								print "<div style='margin:0 0 0 510px;font-weight:bold;'>右 &nbsp;&nbsp;&nbsp; 左</div>";
								print "<div class='clear'></div>";
							}
						print "<li>";
...
function showPatientNIHSSWerte($pnID) {
	...
	if (access()) {
		...
		if ($query_handle != ""){
			...
			if ($rows == 0){
				...
				for($i=0;$i<$rows;$i++){
					...
					if ($query_handle3 != ""){
						...
						if ($id == 99){
							print "<br><b style='color:#FF3300;'>未评分</b>";
							print "";
						}
						print "</li>";
						print "<div class='clear'></div>";
			...
			$printTotal = "<h1 style='border-top:1px solid #000;border-bottom:3px double #000;padding: 13px;'>总评分: $total</h1>";
			}
		} else {
