...
function searchPatientMenu(){
	...
	print "<tr><td><b>Patientenname:</b></td><td><input type='text' name='name' /></td></tr>"; 
    print "<tr><td colspan='2'><b>Geburtstag:</b> ";
    ...
    print "<tr><td colspan='2'><b>Klinikaufnahme</b><br /> ";
	print "von: <input name='from' 	onfocus='showCalendarControl(this);' type='text' size='12' /> ";
	print "bis: <input name='to' 	onfocus='showCalendarControl(this);' type='text' size='12' /></td></tr>";
    print "<tr><td colspan='2'><hr></td></tr>";
	print "<tr><td width='50%'><b>&Uuml;berweiser:</b></td><td>";
	...
	print "</td></tr>"; 
	print "</table>";
	print "<input type='submit' value='Suchen'  style='width:100%;' class='buttonMini' />";
	print "</form>";
	print "</div>";
	print "<div style='float:left;border: 1px dotted #999;margin: 3px;padding: 5px 3px;'>";
	print "<b class='mini'>Klinikaufnahme</b><br /> ";
...
function searchYesterdayButton(){
	...
	print "<input type='submit' value='Gestern'  class='buttonMini' style='width:80px;' />";
	print "</form>";
}
...
function searchTodayButton(){
	...
	print "<input type='hidden' name='x' value='1100' />";
	print "<input type='hidden' name='from' value='$from' />";
	print "<input type='submit' value='Heute'  class='buttonMini' style='width:80px;'  />";
	print "</form>";
}
...
function searchAll($search) {
	...
	if (access()) {
		...
		if ($search[2] <> ''){
			print "<li style='float:left;margin-right: 25px;'>Patientenname: $search[2]</li>";
		}
									...
									if ($query_handle3 != ""){
										...
										if ($rows3 > 0){
											...
											if ($print2){
												...
												print "$pGender $pLastName,  $pFirstName (geb.: $pBdayDay.$pBdayMonth.$pBdayYear$info)";
		...
		if ($query_handle1 != ""){
			...
			if ($rows1 > 0){
				for ($i1 = 0; $i1 < $rows1; $i1++){
					...
					print "<li style='margin-bottom:5px;'>";
					print "<input type='radio' name='patientID' value='$patientID' /> ";
					print "$pGender $pLastName,  $pFirstName (geb.: $pBdayDay.$pBdayMonth.$pBdayYear)";
					$db_request3	 = "SELECT  * FROM patientRecords WHERE patientID = '$patientID' ORDER by patientRecordID DESC";
					$query_handle3   = mysql_query($db_request3, $db_handle);
					if ($query_handle3 != ""){
						$rows3 = mysql_num_rows($query_handle3);
