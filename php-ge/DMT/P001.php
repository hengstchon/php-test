	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			if ($rows1 == 0){
				...
				patientsNavigation();
				print "<fieldset>";
				print "<legend>Vorhandene Patieneneintr&auml;ge, sortiert nach Nachname</legend>";
				if ($case == 'dmt'){
					...
					print "<td valign='top' class='mini' width='28%'>Vorname Nachname (Geb.)";
					print "</td>";
				...
				if ($case == 'web'){
			 	...
			 	for ($i1 = 0; $i1 < $rows1; $i1++){
					...
					if ($case == 'dmt'){
						...
						print "<b>$pLastName</b>, $pFirstName  (geb. $pBdayDay.$pBdayMonth.$pBdayYear, id: $patientID) <br>";
						if ($fallAktenAnz > 0 ){
							if ($fallAktenAnz == 1 ){
					...
					if ($case == 'web'){
						...
						print " <b>$pLastName</b>, $pFirstName (geb.: $pBdayDay.$pBdayMonth.$pBdayYear)</li>";
					}
				}
				if ($case == 'dmt'){
					print "</table>";
				...
				if ($case == 'web'){
					print "</ul>";
					print "<p><input type='submit' value='Ausgew&auml;hlten Patienten &uuml;bernehmen' class='buttonHome' /></p>";
					print "</form>";
				}
				print "</fieldset>";
		...
		if ($query_handle1 != ""){
			...
			for ($i1 = 0; $i1 < $rows1; $i1++){
				...
				$pCity		= schreibweise($pCity);
					...
					$pBdayDay		=	$pBday1[2];
				print "<li><input type='radio' name='patientID' value='$patientID' />";
				print " $pLastName,  $pFirstName (geb.: $pBdayDay.$pBdayMonth.$pBdayYear)</li>";
			}
			print "</ol>";
			print "<input type='submit' value='Ausgew&auml;hlten Patienten &uuml;bernehmen' class='buttonHome' />";
			print "</form>";
			print "</fieldset>";
		} else {
...
function addPatientForm($pLastName) {
	...
	print "<legend>Neuen Patienten anlegen:</legend>";
	if ($case == 'dmt'){
		print "<form method='post' action='DMT.php'>";
	}
	if ($case == 'web'){
	...
	print "Nachname: <input type='text' name='pLastName' value='$pLastName' size='17' />";
	print "<input type='hidden' name='x' value='1015' />";
	print "<input type='submit' value='Patient speichern'  style='width:100%;' class='buttonMini' />";
	print "</form>";
	print "</fieldset>";
...
function editPatient($patientID) {
	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			$pCity			= schreibweise($pCity);
			if ($pLastName == ''){
				print "<legend>Patientendaten eingeben (id: $patientID)</legend>";
			} else {
				print "<legend>$pLastName, $pFirstName (geb. $pBdayDay.$pBdayMonth.$pBdayYear | id: $patientID)</legend>";
			}
			print "<input type='hidden' name='patientID' value='$patientID' />";
			print "<table cellspacing='0' cellpadding='0' style='margin: 5px 0px 0px 0px;line-height:200%;'>";
			print "<tr>";
			print "<td width='175'>Anrede:</td>";
			print "<td>";
			if($pGender == 'w'){
				print "<input type='radio' name='pGender' value='w' checked> Frau ";
				print "<input type='radio' name='pGender' value='m'> Herr";
			} else {
				print "<input type='radio' name='pGender' value='w'> Frau ";
				print "<input type='radio' name='pGender' value='m' checked> Herr";
			}
			print "</td>";
			print "</tr>";
			print "<tr><td>Vorname:</td><td> <input name='pFirstName' value='$pFirstName' /></td></tr>";
			print "<tr><td>Nachname:</td><td> <input name='pLastName' value='$pLastName' /></td></tr>";
			print "<tr><td>Geb.Datum (TT.MM.JJJJ):</td><td>";
			if ($pBday == "0000-00-00"){
				print "<p style='float:left;margin:11px 5px;'>Bitte ausw&auml;hlen: </p>";
			}
			print "<select name='pBdayDay' style='float:left;margin-right: 5px;'>";
			if ($pBdayDay == "00"){
				print "<option selected value=''>Tag</option>";
			} else {
			...
			if ($pBdayMonth == "00"){
				print "<option selected value=''>Monat</option>";
			} else {
			...
			if ($pBdayYear == '0000'){
				print "<option value=''>Jahr</option>";
			}  else {
...
function showPatient($patientID) {
	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			$pBdayDay		=	$pBday1[2];
			print "<legend>$pLastName, $pFirstName (geb.: $pBdayDay. $pBdayMonth $pBdayYear)</legend>";
		} else {
 	...
 	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			if ($rows1 == 0){
				...
				print "<p>Sortiert nach Konsilschein-Erstelldatum & -zeit ";
				print "(Neuste zuoberst -> Patienten mit mehreren Konsilscheinen meist weiter unten, ";
				print " da der erste vor l&auml;ngerer Zeit erzeugt wurde.)</p>";
				foreach ($patientIDs as $key => $patientID){
					...
					print "<fieldset>";
					print "<legend>$nname, $vname (geb. $bDay, id: $patientID)</legend>";
...
function listPatientRecords($patientID) {
	global $db_handle;
	global $case,$x;
	print "<h2>Patienten-Navigation</h2>";
	if (access()) {
		if($case == 'dmt'){
			print "<form method='post' action='DMT.php'>";
		}
		...
		print "<input type='hidden' name='x' value='1020' />";
		print "<input type='hidden' name='patientID' value='$patientID' />";
		print "<input type='submit' value='Patienten-Daten' class='buttonMini' />";
		print "</form>";
		...
		if ($query_handle3 != ""){
			...
			for ($i3 = 0; $i3 < $rows3; $i3++){
				...
				$nr = $rows3 - $i3;
				print "$nr. Dokumentation --- Klinikaufnahme: $time </h3> ";
				print "<p class='mini'>Anfordernder Arzt: $diagnosisArzt<br>";
				if ($therapyArztID <> 0){
					print "Untersuchender Arzt: $therapyArzt";
				}
				print "</p>";
				print "<div class='clear'></div>";
				print "<div class='gruppe'>";
				print "<h4 class='mini'>Konsilschein (ID: $patientRecordID)</h4>";
...
function editPatientRecordDiagnose($patientRecordID) {
	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			print "<tr>";
			print "<td width='175'>Anfordernde Klinik - Arzt: </td>";
			print "<td>";
			if ($case == 'dmt'){
				...
				if ($diagnosisArztID <> 0){
					...
					print "<option  selected>Bitte ausw&auml;hlen</option>";
				}
			...
			print "</tr>";
			print "<tr>";
			print "<td>Beginn der Symptomatik:</td>";
			print "<td>";
			print "<div style='float:left;margin:10px 5px 10px 0px;'>Datum:  </div>";
			...
			print "</div>";
			print "<div style='float:left;margin:10px 5px 10px 0px;'>Zeit:</div>";
			...
			print "<td>";
			if ($symptomsText2 == '4') {
				print "<input type='radio' name='symptomsText2' value='4' checked /> Bekannt &nbsp;  ";
			} else {
				print "<input type='radio' name='symptomsText2' value='4' /> Bekannt &nbsp;  ";
			}
			if ($symptomsText2 == '1'){
				print "<input type='radio' name='symptomsText2' value='1' checked /> Unbekannt &nbsp;  ";
			} else {
				print "<input type='radio' name='symptomsText2' value='1' /> Unbekannt &nbsp;  ";
			}
			if ($symptomsText2 == '2'){
				print "<input type='radio' name='symptomsText2' value='2' checked /> Im Schlaf &nbsp;  ";
			} else {
				print "<input type='radio' name='symptomsText2' value='2' /> Im Schlaf &nbsp;  ";
			}
			if ($symptomsText2 == '3'){
				print "<input type='radio' name='symptomsText2' value='3' checked /> Sonstiges &nbsp;  ";
			} else {
				print "<input type='radio' name='symptomsText2' value='3' /> Sonstiges &nbsp;  ";
			}
			print "<input type='text' name='symptomsText' value='$symptomsText' size='50' />";
			print "</td>";
			print "</tr>";
			print "<tr>";
			print "<td>Zuletzt gesund gesehen:</td>";
			print "<td>";
			print "<div style='float:left;margin:10px 5px 10px 0px;'>Datum:  </div>";
			print "<div style='float:left;margin:0px 15px 0px 0px;'>";
			...
			print "<tr>";
			print "<td valign='top'>Aktuelle Anamnese: </td><td>";
			...
			print "</td>";
			print "</tr>";
			print "<tr><td>Vorerkrankungen/<br>Risikofaktoren:</td><td>";
			...
			print "</td></tr>";
			print "<tr>";
			print "<td colspan='2'>Indikationskodierung:";
...
function editPatientRecordTherapy($patientRecordID) {
	...
	global $case;
	print "<h1>$therapyButton eingeben</h1>";
	if (access()) {
		if ($case== 'web'){
			$arztID	= $_SESSION['arztID'];
		}
		...
		if ($query_handle1 != ""){
			...
			print "</tr>";
			print "<tr>";
			print "<td>Art des Konsils:</td>";
			print "<td>";
			if ($konsilType == ''){
				print "<input type='radio' name='konsilType' value='t'> Telefon &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='b'> Bild und Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='v'> Video ";
			}
			if ($konsilType == 't'){
				print "<input type='radio' name='konsilType' value='t' checked> Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='b'> Bild und Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='v'> Video";
			}
			if ($konsilType == 'b'){
				print "<input type='radio' name='konsilType' value='t'> Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='b' checked> Bild und Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='v'> Video";
			}
			if ($konsilType == 'v'){
				print "<input type='radio' name='konsilType' value='t'> Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='b'> Bild und Telefon  &nbsp; &nbsp; &nbsp; &nbsp; ";
				print "<input type='radio' name='konsilType' value='v' checked> Video";
			}
			print "</td>";
			print "</tr>";
			print "<tr><td valign='top'>Klinischer Befund:</td><td>";
			...
			getTotalNIHSSWerte($patientRecordID);
			print "<br>Weiteren NIHSS-Wert eintragen: <input type='text' name='nihssTotal' size='3' /><br>";
			getNIHSSlist($patientRecordID);
			print "</td>";
			print "</tr>";
			print "<tr>";
			print "<td>Bildgebung:</td>";
			print "<td>";
			if ($visualData[0] == 'on'){
			...
			print " &nbsp; &nbsp; &nbsp;  &nbsp; &nbsp; &nbsp; ";
			if ($visualData[2] == 'on'){
				print "<input type='checkbox' name='visualData[2]' checked> Video &nbsp; &nbsp; &nbsp; &nbsp; ";
			} else {
				print "<input type='checkbox' name='visualData[2]'> Video &nbsp; &nbsp; &nbsp; &nbsp; ";
			}
			print "</td>";
			print "</tr>";
			...
			print "</td>";
			print "</tr>";
			print "<tr><td valign='top'><b>Verdachtsdiagnose:</b></td><td>";
			...
			<b>Beurteilung und Therapieempfehlungen:</b>
			<br />
			...
			print "</tr>";
			print "<tr>";
			print "<td><b>Diagnosekodierung:</b>";
			print "</td>";
			print "<td>";
			...
			print "<h3>STATUS</h3>";
			print "<p>";
			print "<input type='radio' name='editStatus' value='o'> offen (Therapie-Empfehlung wird noch erg&auml;nzt)<br>";
			print "<input type='radio' name='editStatus' value='t' checked> abgeschlossen (Therapie-Empfehlung vorhanden)<br>";
			print "<input type='submit' value='Therapie-Empfehlung speichern' class='buttonHome' />";
			print "</form>";
			print "</fieldset>";
		} else {
...
function showPatientRecord($patientRecordID){
	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			$visualData	= explode(',',$visualData);
			print "<div id='ks'>";
			print "<h2>Konsilschein  (ID: $patientRecordID)</h2>";
			if ($editStatus == "o"){
				print "<h3>";
				print "  - Status: ";
				print "  <img src='imagesLayout/blinkenRot.gif'> Offen ";
			} else {
				print "<h3 id='show'>";
				print "  - Status: ";
				print " Abgeschlossen";
			...
			if ($therapyArztID == 0){
				...
				print "<tr>";
				print "<td width='175'>Untersuchender Arzt:</td>";
				print "<td>$infoDA2</td>";
				print "</tr>";
				...
				$konsilTypeText = '';
				if ($konsilType == 't'){
					$konsilTypeText	= "Telefon";
				}
				if ($konsilType == 'b'){
					$konsilTypeText	= "Telefon & Bild";
				}
				if ($konsilType == 'v'){
					$konsilTypeText	= "Video";
				}
				$controll1 = 0;
				if ($visualData[0] == 'on'){
					$ctOptionText	= "CT: Ja, ";
				...
				if (($controll1 == 1) OR ($controll2 == 1)){
					...
					print "<td>";
					if ($konsilTypeText <> ''){
						echo "Art des Konsils:  $konsilTypeText, ";
					}
					if ($controll2 == 1){
						print "Bildgebung: $ctOptionText $mrtOptionText $videoOptionText $angioOptionText";
					}
					print "</td>";
					print "</tr>";
				}
				...
				if ($therapyDescr <> ''){
					print "<tr>";
					print "<td valign='top' class='borderUnten'>Klinischer Befund: </td>";
				...
				if ($therapyDescr2 <> ''){
					print "<tr>";
					print "<td valign='top' class='borderUnten'>Verdachtsdiagnose: </td>";
					print "<td valign='top' class='borderUnten'>$therapyDescr2</td>";
					print "</tr>";
				}
				if ($therapyDescr3 <> ''){
					print "<tr>";
					print "<td valign='top' class='borderUnten'>Therapieempfehlungen: </td>";
...
function showPatientRecordDiagnose($patientRecordID, $case){
	...
	if (access()) {
		...
		if ($query_handle1 != ""){
			...
			print "<table cellspacing='0' cellpadding='0' style='margin: 5px 0px 0px 0px;line-height:200%;width:100%;'>";
			print "<tr>";
			print "<td width='175'><b>Anfordernde Klinik:</b></td>";
			print "<td>$clinicName ($clinicInitial) - Arzt: $infoDA";
			print "</td>";
			print "</tr>";
			print "<tr>";
			print "<td valign='top'>Zeitangaben:</td>";
			print "<td>";
			print "<b>Beginn der Symptomatik:</b> ";
			print "$timeSymptoms ";
			if ($symptomsText2 == 1){
				print " (Unbekannt) ";
			}
			...
			print "<br />";
			print "<b>Zuletzt Gesund gesehen:</b> $timeSymptomsGesund";
			print "<br />";
			print "<b>Klinikaufnahme:</b> $timeHospital ";
			print "<br />";
			print "</td>";
			print "</tr>";
			if ($symptomDescr <> ''){
				print "<tr>";
				print "<td valign='top'>Aktuelle Anamnese: </td>";
			...
			if ($indication <> '') {
				print "<tr>";
				print "<td>Indikationskodierung:</td>";
				print "<td>";
