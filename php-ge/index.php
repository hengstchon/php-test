...
if (isset($_SESSION['userID']) == true) { 
	...
	if ($_SESSION['userID'] <>'') {
		if ($x == 200) { 
			...
			session_unset();
			session_destroy();
			print "Sie haben den gesch&uuml;tzten Arbeitsbereich verlassen.<br>";
			userLogin('', '');
		} else {
			$xWas = $_SESSION['xWas'];
			if ($xWas <> $x) {
			...
			if ((($x == 3316) OR ($x == 3216)) OR (($x == 3416) OR ($x == 3416))){ 
				...
				print "Datum: $datum, ";
				print "Angemeldeter Arzt: $arztInfos";
				print "</div>";
			} else { 
				print "<div id='infoOben'>";
				print "<form method='post' action='verwaltung.php'>";
				print "<input type='hidden' name='x' value='200' />";
				print "<input type='submit' value='Logout'  class='logout' />";
				print "</form>";
				print "Datum: $datum, ";
				print "Angemeldet: $arztInfos";
				print "</div>";
				navigation();
			}
			...
			if ($x == 100){
				if ($arztName == ''){
					...
					print "<h1><img src='imagesLayout/blinkenRot.gif'> Offene Konsilschein-&Uuml;bersicht</h1>";
			...
			if (($x >= 1000) and ($x < 9000)) {	 
				switch ($x) {
					case 1000: 
						print "<h1>Neuen Patieneneintrag</h1>";
						print "<fieldset>";
						print "<legend>Patieneneintrag anlegen</legend>";
						print "<form method='post' action='verwaltung.php'>";	
						print "<input type='hidden' name='x' value='1010' />";
						print "<p>Nachname des Patienten eingeben: <input type='text' name='pLastName' /></p>";
						print "<p><input type='submit' value='Patienten eintragen'  class='buttonHome' /></p>";
						print "</form>";			
						print "</fieldset>";	
					...
					case 1010: 
						...
						if($exists  == 1){
							...
							if ($addOption == 0){ 
								...
								print "<h1>Patienten-Daten eingeben - bearbeiten</h1>";		
								print "<form method='Post' action='verwaltung.php'>";
								print "<fieldset>";		
								print "<input type='hidden' name='x' value='1030' />";
								editPatientAndDiagnose($patientID);
								print "<input type='submit' value=' >>> Konsilschein senden >>> ' class='buttonHome' />";  
								print "</fieldset>";				
								print "</form>";
							} else {
								print "<h1><img src='imagesLayout/blinkenRot.gif'> Offene Konsilschein-&Uuml;bersicht</h1>";
					...
					case 1020: 
						if ($patientID == ''){
							print "<h1>Patienten-&Uuml;bersicht</h1>";
					...
					break;	 
					case 1025: 
						print "<h1>$diagnoseButton eingeben</h1>";
						print "<fieldset>"; 
					...
					break;
					case 2000: 
						print "<h1><img src='imagesLayout/blinkenRot.gif'> Offene Konsilscheine</h1>";
						listAllPatientsRecords('o') ; 
					break; 
					case 2100: 
						print "<h1>Abgeschlossene Konsilscheine</h1>";
