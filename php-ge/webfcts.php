...
function userLogin($userLogin, $userPW) {
		...
		print "<h2>Bitte loggen Sie sich ein:</h2>";
		print "<table cellspacing='3' cellpadding='0'>";
		print "<tr>";		
		print "<td>Benutzer-Name: </td>";
		...
		print "</tr>";
		print "</table>";
		print "<p><input type='submit' value='Anmelden'  class='submit' /></p>";
		print "</form>";
...
function navigation() {
	...
	if ((($x == '2000') OR ($x == '2100')) OR ($x == '3000')){
			$navSearch = "<form id='navP'><a href='#suchen'>Suche</a></form>"; 
	} else {
			$navSearch = "";
	}
	print "$navSearch"; 
	...
	if (( (( (( (( ((	($x == '') 
			...
			$navPatient = " <form id='navP'><a href='#navPatient'>$pNavSign Patient</a></form>"; 
	}
	print "$navPatient";
		print "</div>";
}
...
function navPatient($patientID) {
	...
	print "<fieldset>";
	print "<legend id='navP2'>$pNavSign Navigation f&uuml;r Patient $nname, $vname: </legend>";
	...
	if (access()) {
		...
		if ($query_handle != "") {
			...
			if($arztGender == 'w'){
				...
				print "<option value='m'>Herr</option>";
			} else {
				$arztGenderText = "Herr ";
				print "<option value='$arztGender' selected>$arztGenderText</option>";
				print "<option value='w'>Frau</option>";
			}
			print "</select>";
			print " &nbsp; Title:";
...
function addRecordForm($patientID) {
	...
	print "<input type='submit' value='Konsilschein Neu' class='buttonNew' />";
	print "</form>";
...
function addThrombolyseForm($patientID,$patientRecordID) {
	...
	print "<input type='hidden' name='patientID' value='$patientID' />"; 
	print "<input type='hidden' name='patientRecordID' value='$patientRecordID' />";
	print "<input type='submit' value='Thrombolyse Neu' class='buttonNew' />";
	print "</form>";
}
...
function editThrombolyseForm($patientID, $ptID) {
	...
	print "<input type='submit' value='Thrombolyse (id: $ptID)' class='$buttonClass' />";
	print "</form>";
	print "<div class='clear'></div>";
}	  
		...
		if ($query_handle1 != ""){
			...
			if ($rows1 == 0){
				...
				for ($i1 = 0; $i1 < $rows1; $i1++){ 
					...
					print "</td>";
					print "<td>";
					print "<div style='float:left;margin:10px 5px 10px 0px;'>Uhrzeit: </div>";
